...
Set `COPILOT_ENABLED=true`, `COPILOT_MODE=automatic`, `DEVICE_ACTIONS=real`, and `OPENAI_API_KEY` on the backend to generate conversation advice directly with OpenAI and play it on the phone without MCP. Keep the API key on the backend; never add it to the Android build. `OPENAI_MODEL` defaults to `gpt-4.1-mini`. On the phone, start a session, enter the conversation situation and one goal, then tap `Enable copilot` to grant `read:context`, `read:transcript`, and `act:audio` for that session. A watch `Ask copilot` tap sends the context and latest 20 transcript segments to OpenAI with response storage disabled, then queues its concise advice through the existing audio path. If OpenAI is unavailable, the backend falls back to deterministic goal-based advice. Set `COPILOT_MODE=mcp` only when an MCP host should generate the advice through `handle_copilot_request` instead.
